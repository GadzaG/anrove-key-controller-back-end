...
export class StatisticsService {
	...
	> {
		...
		for (const reg of registrations) {
			const month = reg.createdAt.getMonth() + 1, // Получаем месяц создания (от 1 до 12)
				year = reg.createdAt.getFullYear(), // Получаем год создания
				key = `${year}-${month}` // Создаем ключ в формате "год-месяц"
		...
		const returnedMonths = allMonths.map(({ month, year }) => {
	...
	private generateMonths(
		...
		const current = new Date(start), // Копируем начальную дату (Июнь прошлого года)
			endMonth = new Date(end), // Копируем конечную дату (Конец текущего месяца)
			months = [] // Массив для хранения месяцев
